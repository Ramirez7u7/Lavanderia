...
export default function ClientsView() {
  ...
  const searchName = async () => {
    try {
      ...
      const response = await fetch(`https://kh0tkmpw-5000.usw3.devtunnels.ms/clients/search/name?name=${encodeURIComponent(name)}`, {
        method: "GET",
  ...
  return (
    <View style={styles.container}>
      <View>
        <Text style={styles.title}>Hola, que necesitas ?</Text>
        <Pressable style={styles.send}>
          <Link href="/clientes/createC" style={styles.textButton}>Crear Cliente</Link>
        </Pressable>
        <Pressable style={styles.sends} onPress={searchName}>
